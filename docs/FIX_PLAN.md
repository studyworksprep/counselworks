...
**Status:** Phases 0–13 implemented; migrations through 00041 applied in
...
| # | Item | Details |
|---|------|---------|
| 13.0 | Student workspace | Per-student sub-pages under one header + sub-navigation (Overview, Profile, Colleges, Applications, Essays, Tasks, Documents, Meetings, Family & Billing) and a class-year student rail beside them — the rail (search + Add student) is the way to find and switch students, on the roster and inside a student alike, so a counselor works one student end to end without bouncing through the firm-wide lists. Both the main sidebar and the rail collapse (remembered per browser) to save width and to keep the rest of the roster off a shared screen. Sub-pages reuse the global list components pinned to the student (no parallel implementations). Billing stays on the family; the student's Family & Billing page is read-only with links. Families get the same treatment: a household rail (alphabetical — a family has no class year of its own; search + Add family, collapsible) and a family workspace with Overview (members, students, contact, notes), Billing (agreement + invoices), Tasks, Documents, and Meetings sub-pages, sharing the rail/switcher/meeting-list components with the student workspace. |
| 13.1 | Meeting self-booking | **Shipped.** Each staff member publishes weekly windows (wall-clock in their own IANA zone) and rules (slot length, minimum notice, horizon, default location) from Settings → Booking Availability. Parents open Book a Meeting in the family portal, choose a child and one of that child's assigned counselors who has published availability, and pick from open slots computed server-side (windows minus the counselor's existing meetings, `src/lib/booking/slots.ts`) and rendered in the parent's browser zone. Booking re-verifies the slot server-side and writes an ordinary family-visible meeting (`booking_source = 'portal'`, migration 00040 admits exactly that shape under RLS) with counselor + parent attendees (and the student when they hold a portal account), an audit event, an in-app notification and email to the counselor, and a confirmation email to the parent. The meeting then rides the existing calendar, ICS feed, reminder cron, and portal dashboards; the calendar detail and workspace meeting lists mark it "Booked by family". Parents cannot edit or delete meetings — rescheduling goes through the counselor. Golden-path step 6b covers it. |
| 13.2 | Scattergrams / historical outcomes | **Shipped.** Every college detail page (`/college-planning/[id]#admissions-history`) carries the firm's own decision history: one mark per decided application, test score (SAT scale; an ACT-only student is concorded with the official 2018 table and the tooltip says so) × GPA (unweighted, else weighted and labeled), outcome encoded twice — status color and shape — with a fixed legend, per-mark hover tooltip, class-year filter, and a table view; decisions missing a score or GPA are counted, not dropped. No schema: the source is the 10.2 decision roster (`applications.decision_result` + `students` GPA + `student_profiles` scores, `src/lib/colleges/scattergram.ts`), and the Reports roster links each college name to its scattergram. Staff see the whole firm's points (the history that compounds each season) but a student's name only where they may access that student. Students and parents get the same chart, collapsed under "Admission history at this college" on each college card, anonymized (no ids, names, or class years, order re-keyed by score) with a "You" marker for their own student — and only once the college holds at least 3 recorded decisions, a deliberate privacy floor below which the card says so instead. Golden-path step 13b covers the named staff point, the roster link, and the portal gate. |
| 13.3 | Recurring tasks | **Shipped.** Staff save a recurring task template from Tasks → New recurring task (also inside the student and family workspaces, pinned to the student/household): title, description, priority, type, weekly-on-a-weekday or monthly-on-a-day (1–28), start date, assignee (default: the student's primary counselor, then the template author), related student, and the same audience control as a one-off task (staff / student / student + family; portal-visible templates require a student). `recurring_task_templates` (migration 00041) is staff-only under RLS; the materializer (`src/lib/tasks/materialize-recurring.ts`) turns each due occurrence into an ordinary `tasks` row in the shape the workflow task-sync writes — plus `recurring_template_id` / `occurrence_on`, whose partial unique index makes every re-run a no-op — due at 09:00 firm-local on the occurrence date (`firms.timezone`). Two callers: the create / resume / edit actions run it for that template immediately (the first occurrence exists the moment the counselor saves), and the `recurring-tasks-materialize` Inngest cron (every six hours) runs it for every firm, resuming from each template's `last_materialized_on` and never catching up more than eight weeks after a pause. Generated tasks carry a Recurring badge on the staff task table and flow through the existing student and family task pages by visibility; templates never reach the portals. Templates can be edited (every field defaulted from the row), paused / resumed, and archived; audit events record each change. Golden-path step 8b covers it. |
| 13.4 | CSV bulk import | **Shipped.** Students roster / Families roster → Import CSV (owner/admin only, the 7.1 intake gate) at `/students/import`: one row per student with household, optional school/address, one parent (name + email + relationship), and the email of the staff member to assign; downloadable template; spreadsheet-friendly header aliases. Preview is a server-side dry run (`previewClientImport`) that plans every row against the firm — Create / Existing / Already exists / Assign / Unknown staff — and lists per-row validation errors; Import (`runClientImport`) re-parses and re-plans the same file, refuses while any row is invalid, then creates households, students (through the shared `provisionStudent`, so profile + default workflow match the form), parent placeholders (same `invited_` users addFamilyMember creates; first parent becomes primary contact), and counselor assignments, and records one audit event. Idempotent: households match by name, students by household + name + class year, parents by email, assignments by student + staff — re-importing creates nothing. Golden-path step 1b covers preview, import, and the idempotent re-run. |
...
Release follow-up: [PR #40](https://github.com/studyworksprep/counselworks/pull/40) is open. Migration 00043 was applied and schema/RLS verified on CounselWorks as `20260908202359_task_owner_intent` under explicit user authorization. No backfill or app deployment. Existing security-advisor warnings are unchanged; see the [migration record](WORKFLOW_IMPROVEMENT_PLAN.md#pr-and-migration-follow-up--september-8-2026). Live acceptance remains open.

Phase C follow-up: [implementation record](WORKFLOW_IMPROVEMENT_PLAN.md#phase-c-implementation-record--september-8-2026). PR #40 is merged; local branch `codex/workflow-phase-c` adds exact deliverable review, the counselor review queue, atomic task/step transitions, preserved downstream work on reopen, and transactional/idempotent materialization. Migration 00044 remains unapplied to production. Type-check/lint and 242 unit tests pass; fresh migration/isolation/review suites and 12 concurrent materialization attempts pass. All 25 E2E tests skip without Clerk development keys. Live acceptance remains open; next implementation is Phase D.

Phase D follow-up: [implementation record](WORKFLOW_IMPROVEMENT_PLAN.md#phase-d-implementation-record--september-9-2026). Shared editable plan previews now serve student workspaces, templates, cohorts, and college supplements. Saved settings, scoped duplicate prevention, explicit repeat plans, accepted application-schedule changes, and firm-calendar task dates are implemented locally. Migration 00045 (after 00044) remains local. Legacy settings preservation is an explicit per-plan action; no automatic historical owner/date/completion backfill. Live acceptance remains open; next implementation is Phase E notifications and audit handoff.

Phase E follow-up: [implementation record](WORKFLOW_IMPROVEMENT_PLAN.md#phase-e-implementation-record--september-9-2026). Phases C–D are open in [PR #41](https://github.com/studyworksprep/counselworks/pull/41); Phase E is isolated on `codex/workflow-phase-e`. Transactional notices/audit, actual-recipient checks, consolidated plan publication, task email preferences, overdue/reviewer reminders, current next-action context, and durable retry receipts are implemented locally. Migration 00046 follows 00044–00045; no production changes or backfill. Local database/handler tests pass, while all 25 browser tests skip and real Inngest scheduler/email acceptance remains open. Next: combined live acceptance and release review, not another implementation phase.


## UX/UI follow-up — September 9, 2026

[UX1 implementation and verification record](UX_UI_IMPLEMENTATION_PLAN.md#ux1-implementation-record--september-9-2026): staff-only roster/count and guarded membership actions, plus consistent global My Tasks ownership, are implemented on `codex/ux1-roles-task-ownership` from merged main `cb7ccb0`. Type-check/lint, 287 unit tests, webpack production build, disposable migration/isolation/regression SQL suites, and four independent live UX1 role tests pass. Full browser regression remains blocked at upload by missing local Inngest configuration (10 passed, 1 failed, 3 skipped, 11 not run); Stripe and actual email delivery remain unverified. No UX1 migration or backfill is needed and no deployment/production migration was performed. **At the UX1 handoff, next was UX2**; see the subsequent record below.

[UX2 implementation and verification record](UX_UI_IMPLEMENTATION_PLAN.md#ux2-implementation-record--september-9-2026): responsive entity rails, student summary/body grids, labeled narrow task cards, wrapping workflow/assignment controls, and keyboard section overflow are complete locally on `codex/ux2-responsive-workspaces`, based on UX1 `2a4a886`. Type-check/lint, 287 unit tests, and webpack build pass. All eight UX1/UX2 live role checks pass; owner/counselor use all five planned widths, student/parent use phone and desktop. Full configured suite: **18 passed, 1 failed (upload/Inngest), 3 skipped, 11 not run**. No migration/backfill is required. Existing phone invoice wrapping is recorded for UX5. No push/deployment/production migration was performed. **At the UX2 handoff, next was UX3**; see the subsequent record below. Broader workflow/email/payment gates remain open.

[UX3 implementation and verification record](UX_UI_IMPLEMENTATION_PLAN.md#ux3-implementation-record--september-10-2026): shared status filters, counted staff review entries, independent student personal/waiting previews, permission-checked missing-task recovery, matching weekly workflow metrics/destinations, counselor plan navigation, parent College Lists naming, readable canonical message references, and date-specific timezone labels are implemented locally on `codex/ux3-task-navigation` from merged main `80d6d43`. Type-check/lint, 297 unit tests, webpack build, and disposable migration/isolation/workflow SQL suites pass. Three targeted live role journeys passed, including actual staff recovery and cleanup of generated records; owner review entry was checked manually. Final configured browser suite: **21 passed, 1 failed (upload/Inngest), 3 skipped, 11 not run**; all eleven UX1–UX3 live checks, staff axe, and booking passed. No migration/backfill is required; no deployment or production migration was performed. **At the UX3 handoff, next was UX4**; see the subsequent record below. Broader email/payment/workflow release gates remain open.


[UX4 implementation and verification record](UX_UI_IMPLEMENTATION_PLAN.md#ux4-implementation-record--september-10-2026): collapsible plan summaries, persistent non-overlay actions, wider assignment dialogs with keyboard focus containment, retained invalid/failed-request edits, readable metadata, and confirmation links to actual student workflow instances are complete locally on `codex/ux4-plan-preview`, based on unpushed UX3 `af4af15`. Type-check/lint, 297 unit tests, webpack build, and disposable migration/isolation/workflow SQL suites pass. All four targeted UX4 role journeys passed with cleaned-up local fixtures and five-/twelve-step phone/desktop review. Final configured browser suite: **25 passed, 1 failed (upload/Inngest), 3 skipped, 11 not run**; all fifteen UX1–UX4 role checks, staff axe, and booking passed. Global stale external-account cleanup was disabled after automatic approval review rejected it; `E2E_SKIP_STALE_CLEANUP=1` is documented in E2E.md. No migration/backfill is required, and no push/deployment/production migration was performed. **At the UX4 handoff, next was UX5**; see the subsequent record below. Broader release gates remain separate.

### September 10 production dashboard incident

After merged PR #45, the owner dashboard failed with `Unable to load review queue` (Vercel digest `2062702058`, Supabase HTTP 400). A read-only check of production project `bfgiiapopzexrrcpsmyh` confirmed `tasks.reviewer_user_id` and `tasks.needs_attention` are absent; migration history lacks workflow migrations 00044–00046. The new dashboard review-count query exposed that existing application/schema mismatch. Local verification used migrations through 00046 and did not establish production compatibility.
...
## UX5 family priorities — September 11, 2026

[UX5 implementation and verification record](UX_UI_IMPLEMENTATION_PLAN.md#ux5-implementation-record--september-11-2026): child-first family dashboard, compact signature/payment notices, dedicated billing and portal preferences, separated personal/firm settings, readable phone invoices, and day-based booking with nearby confirmation are complete locally on `codex/ux5-ux6-family-acceptance`. All seven UX5 live checks passed using owner, counselor, student, and both parents with a second-child fixture; booking conflict recovery preserved notes/attendees and the selected parent alone could complete their task. Type-check/lint, 301 unit tests, production build, and disposable migration/isolation/workflow SQL suites passed. No migration/backfill or production change is required. **Next at this phase commit: UX6 — Integrated acceptance and handoff**, authorized in the same session.


## UX6 integrated acceptance — September 11, 2026

[UX6 implementation and verification record](UX_UI_IMPLEMENTATION_PLAN.md#ux6-implementation-record--september-11-2026): **UX1–UX6 are complete locally.** All 29 dedicated role tests passed; final configured full browser suite is **51 passed, 3 Stripe skips, zero failures/unreached tests**. Local Inngest enabled the previously blocked upload/document-review and downstream golden path. The populated essay journey now verifies revision, resubmission, approval, the actual dependent next action, private staff visibility, both parents, and a second child. Integrated acceptance also fixed application calendar dates shifting a day across timezones; staff, portal, and print views share the calendar-date formatter. Type-check/lint, **305 unit tests**, webpack build, and disposable isolation/workflow SQL suites passed. No migration/backfill or production change is required. **Exact next phase: none in this UX plan; next delivery step is PR review.** Actual scheduled notification/email receipts, Stripe onboarding/payments, and deployed role smoke checks remain independent release gates. No deployment or new production migration was performed.

...
const state = vi.hoisted(() => ({ actor: "student", role: "student", updates: 0,
  task: { id: "task", student_id: "child", visibility_scope: "family", assigned_user_id: "counselor", created_by_user_id: "counselor", status: "pending", task_type: "general", owner_pending: false } }));
...
vi.mock("@/lib/workflows/tasks-sync", () => ({ completeStepForCompletedTask: async () => ({ error: null }), unlinkTaskFromAnyStep: async () => ({ error: null }) }));
...
beforeEach(() => { state.actor = "student"; state.role = "student"; state.updates = 0; state.task.assigned_user_id = "counselor"; state.task.task_type = "general"; state.task.owner_pending = false; });

...
export function StudentRail({
  ...
  if (collapsed) {
    return (
      <aside
        aria-label="Students"
        className="hidden w-10 shrink-0 border-r border-gray-200 bg-white lg:block"
      >
  ...
  return (
    <aside
      aria-label="Students"
      className="hidden w-60 shrink-0 border-r border-gray-200 bg-white lg:block"
    >
      <div className="sticky top-0 flex max-h-screen flex-col">
        <div className="flex items-center justify-between px-3 pt-3">
          ...
          <div className="flex items-center gap-1">
            <Link
              href="/students"
              className="rounded-md px-1.5 py-1 text-xs text-primary-600 hover:bg-gray-100 hover:text-primary-700"
            >
              Roster
            </Link>
            <button
        ...
        <div className="space-y-2 px-3 py-2">
          <input
            ...
            className="w-full rounded-md border border-gray-200 px-2 py-1.5 text-sm placeholder:text-gray-400 focus:border-primary-400 focus:outline-none"
          />
          {canCreate && (
            <Link
              href="/students/new"
              className="block rounded-md border border-dashed border-gray-300 px-2 py-1.5 text-center text-xs font-medium text-gray-600 hover:border-primary-400 hover:text-primary-700"
            >
              + Add student
            </Link>
          )}
          ...
          {years.map((year) => (
            ...
            >
              <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                ...
                <span className="text-xs text-gray-400">{byYear.get(year)!.length}</span>
              </summary>
              <ul className="pb-2">

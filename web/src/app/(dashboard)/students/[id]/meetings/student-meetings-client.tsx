...
/**
 * Student Meetings list (fix plan 13.0). Reuses the calendar's schedule
 * and detail/edit modals so behaviour (timezone handling, attendees,
 * portal visibility, useWriteRefresh repaint) is identical. Captured-
 * object rule: the open meeting is held by id and derived from current
 * props, so a repaint after an edit shows the new values.
 */
...
export function StudentMeetingsClient({
  studentId,
  studentName,
  nowIso,
  meetings,
  ...
  const students = [{ id: studentId, name: studentName }];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <MeetingList items={upcoming} empty="No upcoming meetings scheduled." onOpen={setOpenId} />
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <MeetingList items={past} empty="No past meetings yet." onOpen={setOpenId} />
          </CardContent>
        </Card>
      </div>
